...
CONST_screen_matrix = np.array([[0, 0],[WIDTH, 0],[0, HEIGHT],[WIDTH, HEIGHT]]) - OFFSET # Second term is to center.
...
class Map:
    ...
    def map_cost(self):
        ...
        # Define probability distribution for different costs with higher water probability
        # [zero, one, two, three, four, block (water)]
        # Increased block (water) probability to 0.25 (25%)
        probabilities = [0.05, 0.3, 0.2, 0.1, 0.1, 0.25]
        
        # Create a cumulative distribution for numpy's choice function
        cost_values = [0, 1, 2, 3, 4, float('-inf')]
        
        for h in self.hexes.values():
            # Use numpy.random.choice with probability weights
            chosen_cost = np.random.choice(cost_values, p=probabilities)
            costs[h] = chosen_cost
