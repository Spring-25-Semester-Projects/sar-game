...
class Survivor(Entity):
    def __init__(self, game_map: Map, color=ENTITY_COLOR, health: int = 100, speed: int = 1, ai_type: str = None):
    ...
    def update(self, dt, player: 'Survivor' = None):
        self.hunger += 0.1 * dt
        if self.hunger > 100:
            self.take_damage(1 * dt)
        self.stamina = min(self.stamina + 0.5 * dt, 100)
        if self.ai_type and player:
